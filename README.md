...
第1章 Javaってなんだろう
1.1	Javaとは
-	Java言語と実行環境であるJava仮想マシン(JVM)を中心としたプログラミング技術。
	現在はOracles社が中心となって開発を進めている。

1.1.1	プログラミング言語とは
-	プログラム：コンピュータを動かす命令やデータをまとめたもの。
	ソースコード：プログラムを人間に読み書きできる形にしたもの。
	プログラミング言語：ソースコードの文法などの決めごとを定めた言語。

1.1.2	Javaはどんなプログラミング言語なの？
-	Javaはオブジェクト指向言語だが、近年は関数型プログラミング手法を取り入れている。
-	オブジェクト指向言語：クラスという単位でプログラムをまとめて全体を構築する手法。
	関数型プログラミング：関数を単位にプログラムをまとめる手法。
-	JVM：バイトコードを読み込んで実行する。環境(CPUの種類とOS)ごとに用意されている。

1.1.3	Javaで何ができるの？
-	得意なのは業務システム、Webサービスの構築

1.1.4	Javaの仕様とJDKディストリビューション
-	JDK(Java Development Environment)：Javaのプログラムを開発するのに必要なソフトウェアパッケージ。コンパイラやJVMも含む。
-	Java SE：Javaでどのようにプログラムを書けるか、そしてどのように動作するかといった仕様のこと。
-	OpenJDK：現在、Javaの開発はオープンソースで進められている。
各機能はまずJEP（JDKEnhancementProposal）と呼ばれる提案をベースにして開発され、開発がリリースの期限に間に合ったものが次のバージョンに載るという手続きになっている。
- ビルド：OpenJDKとして開発されたJavaを使う際に実行用パッケージを作ること。
-	ディストリビューション：多くの会社や団体が配布している実行用パッケージのこと。
-	バージョンアップ：Javaは半年に一度、3月と9月に新しいバージョンがリリースされる。
-	3年（6バージョン)ごとにLTS（LongTermSupport）という、数年間メンテナンスが継続されるバージョンが設定される。

1.2	Javaの情報源
-	公式ドキュメント：https://docs.oracle.com/javase/jp/17/
-	日本 Java ユーザー グループ(JJUG)：https://www.java-users.jp/
-	JJUG Twitterアカウント：https://twitter.com/JJUG

第2章 開発環境の準備と最初の一歩
2.1	OracleJDKのインストール
2.1.1	OracleJDKのダウンロード
- 手順の説明なので省略
2.1.2	OracleJDKのインストール
- 省略

2.2	IntelliJ IDEAのインストールと設定
2.2.1	初心者の段階からIDEを！
-　プログラミングにおいて本質的には必要のない苦労を避けて効率よく大変開発を進めるため、初心者からIDEを使いましょう。

2.2.2	IntelliJ IDEAとは
-　海外では古くから名の知れたJava用のIDE。世界シェアナンバー1。
-　チェコ共和国のプラハに本社を置くJetBrains社が開発。

2.2.3	IntelliJ IDEAのダウンロードとインストール
-　JetBrains Toolbox Appというツールを使ってインストールする。
-　Toolbox Appをインストールするには、JetBrains社のサイトからダウンロードする。

2.2.4	ToolboxAppの設定とIDEの起動
-　省略
...
2.3	最初のプログラムを書いてみよう
2.3.1	プロジェクトの作成
-　IntelliJ IDEAではプログラムをプロジェクトとして管理する。
-　プログラム：	アプリケーションを目的単位でまとめたもの。
			プロジェクトに関連するファイルは1つのディレクトリにまとめられる。
			1つのプロジェクトにつき1つのウィンドウが割り当てられる。
-　プロジェクト形式の選択
 　[新規プロジェクト]ウィンドウの左側メニューから[Maven]を選択する。
 　Mavenは「ビルドツール」や「プロジェクト管理ツール」と呼ばれる種類のツールで、Javaではデファクトスタンダードとなっているもの。
 　[ Java モジュール(Java)]はIntelliJ IDEA独自プロジェクト形式なのでお勧めではない。
-	プロジェクトSDK( Project SDK)
 　SDK：	SoftwareDevelopmentToolの略で、コンパイラなど開発に使うツールの総称。
		JavaではJDKがSDKにあたる。
	[<SDKなし（NoSDK）>]と表示されている場合はキャンセルボタンを押してJDKをインストールする。
※インストーラーからJDKをインストールすること。このとき、IntelliJ IDEA内でJDKを直接ダウンロードすると、OSの「パス」が設定されないため、あとで利用するjavacやjshellなどのコマンドが使えなくなる。
...
2.3.3	クラスの作成
-　クラス
 　Javaでプログラムを書くときは最初にクラスを作成する。
 　クラスはJavaプログラムを構成する1つの単位。
-　Maven形式のプロジェクトのはじめかた
 　ソースコードはプロジェクトディレクトリ内のsrc/main/java以下に置く。
 　新規ファイル作成のポップアップを表示する。→Javaクラスを選択する。

2.3.4	mainメソッドの実装
-　クラスができたらプログラムを実行する起点であるmainメソッドを「{」と「}」の間に書く。
 　main()メソッドを宣言する。： public static void main( String[] args){}
 　文字列を画面に表示する命令を書く。： System. out. println()
 　表示する文字列を書く。： " Hello World"
 　書いたプログラムを実行する。： ショートカットキー [Ctrl]+[Shift]+[F10]
